...
=begin
      #NOTE: Right now this functionality disableed (core.rb: line 180 )
...
=end
      ...
      it "should optionally discard page bodies to conserve memory" do
        # core = Dcrawler.crawl(FakePage.new('0').url, @opts.merge({:discard_page_bodies => true}))
      ...
      it "should optionally delay between page requests" do
        ...
        Link.enq(:url => pages[0].url)
        start = Time.now
        Dcrawler.crawl(@opts.merge({:delay => delay}))
        finish = Time.now
      ...
      it "should optionally obey the robots exclusion protocol" do
        ...
        Link.enq(:url => pages[0].url)
        core = Dcrawler.crawl(@opts.merge({:obey_robots_txt => true}))
      ...
      it "should be able to set cookies to send with HTTP requests" do
        ...
        core = Dcrawler.crawl(@opts) do |dcrawler|
          dcrawler.cookies = cookies
        end
      ...
      it "should freeze the options once the crawl begins" do
        ...
        core = Dcrawler.crawl(@opts) do |dcrawler|
          dcrawler.threads = 4
          dcrawler.on_every_page do
            lambda {dcrawler.threads = 2}.should raise_error
          end
        end
      ...
      describe "many pages" do
        ...
        it "should optionally limit the depth of the crawl" do
          Link.enq(:url => @pages[0].url)
          core = Dcrawler.crawl(@opts.merge({:depth_limit => 3}))
    ...
    describe "options" do
      it "should accept options for the crawl" do
        ...
        core = Dcrawler.crawl(:verbose => false,
                             :threads => 2,
      ...
      it "should use 1 thread if a delay is requested" do
        Link.enq(:url => SPEC_DOMAIN)
        Dcrawler.crawl(:delay => 0.01, :threads => 2, :queue_timeout => 2).opts[:threads].should == 1
      end
    end
  end
end
